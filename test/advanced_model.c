...
bool usercode_loadmedia()
{
  ...
  if (!KRR_TEXTURE_load_texture_from_file(texture, "res/models/white.png"))
  {
  ...
  SU_BEGIN(texture3d_shader)
    ...
    texture3d_shader->reflectivity = 0.8f;
  ...
  if (e->type == SDL_KEYDOWN)
  {
    ...
    if (k == SDLK_RETURN)
    {
      ...
      if (gWindow->fullscreen)
      {
        ...
        // change position adhering to windowed mode
        glm_vec3_copy((vec3){0.0f, 1.0f, 20.0f}, cam.pos);
...
void usercode_render()
{
  ...
  glBindVertexArray(sm->vao_id);
    ...
    glm_translate(texture3d_shader->model_matrix, (vec3){0.0f, -3.0f, 0.f});
