...
static KRR_TEXTURE* dds_texture = NULL;
...
void usercode_render()
{
  ...
  KRR_TEXTURE_bind_vao(dds_texture);
    KRR_SHADERPROG_bind(texture_shader->program);

    glm_mat4_copy(g_base_ui_model_matrix, texture_shader->model_matrix);
    glm_translate(texture_shader->model_matrix, (vec3){g_logical_width/2.f, g_logical_height/2.f, 0.f});
    glm_translate(texture_shader->model_matrix, (vec3){-dds_texture->width/2.f, -dds_texture->height/2.f, 0});

    KRR_TEXTURE_render(dds_texture, 0.f, 0.f, NULL);

    KRR_SHADERPROG_unbind(texture_shader->program);
  KRR_TEXTURE_unbind_vao(dds_texture);
